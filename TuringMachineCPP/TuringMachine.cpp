...
TuringNode::TuringNode(std::string name) : nodeName{ name } {
	std::fill(possibleActions, possibleActions + 256, nullptr);
};


void TuringNode::addConnection(char input, Action* action) {
	possibleActions[input] = action;
}

const Action* TuringNode::read(char input) {
	return possibleActions[input];
}


std::string TuringNode::getName() {
	return nodeName;
}
...
void TuringMachine::writeAt(int index, char write) {
	if (index >= 0) {
...
char TuringMachine::getAt(int index) {
